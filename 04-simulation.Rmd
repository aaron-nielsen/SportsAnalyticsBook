...
sim_goals %>% data.frame %>% ggplot(aes(sim_goals)) + 
  geom_bar() + 
...
table.pa %>% kable(booktabs=T,col.names=c("Plate App.","Freq."),align = c("c","c"))

table.ab <- table(dimaggio$AB)
table.ab %>% kable(booktabs=T,col.names=c("At-Bats","Freq."),align = c("c","c"))
```
...
Running the simulation above with `set.seed(2022)` and `n.sims=10000`, we get $P(Streak \geq 56) = 3 \cdot 10^{-4}$. There were three simulated hitting streaks of at least 56 games.
