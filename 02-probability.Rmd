...
https://cdn2.sportngin.com/attachments/document/c780-2407390/DPFlexRuleExplained.pdf
...
fig1 <- ggplot(sim1.df, aes(sample = x)) +
  stat_qq(position=position_jitter(width = 0.2,height = 0.2)) +
  stat_qq_line() +
...
fig3 <- ggplot(sim1.df, aes(sample = x)) +
  stat_qq(distribution = qpois, dparams = 5,position=position_jitter(width = 0.2,height = 0.2)) +
...
rapids21 %>% 
  ...
  stat_qq(distribution = qpois,
          dparams = 1.5,position=position_jitter(width = 0.2,height = 0.2)) +
...
shaq9798 %>% 
  ...
  ggtitle("QQ-plot for Shaq's Free Throw Attempts vs. Neaative Binomial Distribution") +
  xlab("Theoretical Quantiles") +
  ylab("Empirical Quantiles")
```
...
So, $P(X>Y) = P(X-Y>0) = P(Z>0) = 1-P(Z $\leq$ 0)$
