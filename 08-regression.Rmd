...
(f) Build a MR model between runs scored and OPS, BA, OBP, and OPS.
...
:::{.example}
Ken Griffey Jr. was a baseball hall of fame outfielder from 1989--2009. For each of his of his career, the number of at-bats per home run is record. This data is found in `griffey_hr.csv`. (From \textit{Analytic Methods in Sports})
:::
...
griffey <- griffey %>% mutate(Year = Year-1988,`AB/HR` = HR) %>% select(Year,`AB/HR`)
...
```{r}
model <- lm(`AB/HR`~Year,data=griffey)
model %>% tidy() %>%
  mutate(
    p.value = scales::pvalue(p.value),
  ...
  kable(booktabs=T,digits=c(3,3,3,3), 
        caption = "SLR Model Estimating AB/HR using Year",
        col.names = c("Predictor", "Estimate", "Std Error", "t stat", "p-value")) %>%
  kable_styling(latex_options = "hold_position")
```
...
```{r,warning=F,message=F}
griffey %>% ggplot(aes(x=Year,y=`AB/HR`)) +
  geom_point() +
  geom_smooth(method="lm") + 
  stat_regline_equation(label.x.npc = 0.3,label.y.npc = 0.6) +
  labs(title = "At-Bats per Home Run vs. Year",
...
```{r}
model_quad <- lm(`AB/HR`~Year+I(Year^2),data=griffey)
model_quad %>% tidy() %>%
  mutate(
    p.value = scales::pvalue(p.value),
...
griffey %>% ggplot(aes(x=Year,y=`AB/HR`)) +
  ...
  labs(title = "Ken Griffey Jr.'s Career",
       subtitle = "At-Bats per Home Run vs. Year",
       caption = "Data: Baseball Reference, Analytic Methods in Sports", 
       x = "Year",
       y = "At-Bats Per Home Run") +
  theme_minimal()
```
