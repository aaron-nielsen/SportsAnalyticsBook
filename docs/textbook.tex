...
For statistical inference, we need a point estimator and a measure of uncertainty. We will focus on creating confidence intervals for population mean and population proportion.
...
\textbf{General Form of a Confidence Interval (Two-Sided)}

\vfill

\emph{Note:} \(\alpha\) is the probability that a confidence interval does not contain the true population parameter given that the null hypothesis is true. It is also the probability of a \emph{Type I error}.

\textbf{Writing and Interpreting a Confidence Interval}
