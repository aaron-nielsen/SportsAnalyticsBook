...
### Basic Baseball Hitting Statistics
...
rox21 <- rox21 %>% 
  ...
  mutate(SLG = round(((H-`2B`-`3B`-HR) + 2*`2B` + 3*`3B` + 4*HR)/AB,3)) %>%
  mutate(OPS = round(SLG + OBA,3)) %>%
  mutate(wOBA = round((0.692 * (BB - IBB) + 0.722 * HBP + 0.879 * (H-`2B`-`3B`-HR) + 1.242 * `2B` + 1.568 * `3B` + 2.007 * HR)/(AB + BB - IBB + SF + HBP),3)) %>%
  mutate(ISO = round(SLG-AVG,3))
rox21 %>% select(Name,PA,AVG,OBA,SLG,OPS,wOBA,ISO)
```
:::{.example}
...
### BasketballAnalyzeR Shot Charts
