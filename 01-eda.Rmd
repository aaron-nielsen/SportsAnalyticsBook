...
`r if(add.space){"<br><br><br><br><br><br><br>"}`
...
`r if(add.space){"<br><br><br><br><br><br><br><br>"}`
...
`r if(add.space){"<br><br><br><br><br><br>"}`
...
NFL_2021_Team_Passing$Abbr <- 
  c("TB","LAC","DAL","KC","LAR","LV","CIN","GB","BUF","ARI","MIN","SF",
...
NFL_2021_Team_Passing %>%
  ggplot(aes(x=Yds,y=TD)) +
  scale_x_continuous(limits=c(2750,5250)) +
  nflplotR::geom_nfl_logos(aes(team_abbr = Abbr), width = 0.04, alpha = 0.7) +
  labs(x="Team Passing Yards",y="Team Passing Touchdowns",
       title="NFL Team Passing Yards, 2021")
```
...
# kable table global setup
kt <- function(data) {
  knitr::kable(data, digits=3,linesep='',booktabs=TRUE) %>% kable_styling(latex_options='HOLD_position', full_width = F, position = "center")
}
```
...
<!-- Add WAR calculations, Mathletics, chapter 9, page 92)-->
...
basketball_data <- tibble(team = c("CSU","WYO"), FG = c(14,15), FGA = c(48,60), 
                          THREEP = c(5,4), FT = c(10,4), FTA = c(14,4), 
...
<!-- add from Mathletics, chapter 28, page 248 -->

```{r four factors 2}
basketball_data <- basketball_data %>% 
  mutate(eFG = 100*(FG + .5 * THREEP)/FGA) %>%
  mutate(TOVPCT = 100*(FG + .5 * THREEP)/FGA) %>%
...
ggpairs(xGoal_stats,
        title="Correlation plot of expected goals and relevant predictors, NHL 2021-22")
```
...
<!-- Offensive and Defensive Efficiency, Mathletics, chapter 39, page 358 -->
...
:::{.example}
Use the `worldfootballR` package to collect team shooting data from the Women's 1st Tier League (Women's Super League) in England for 2020-2021. Plot goals vs. shots, shots on target, and expected goals. Comment on the relationships.
:::
