...
On any given football play, a team has an expected point value (from the current drive) before the play and an expected point value after the play. The difference between these expected values, is the change in expected points that resulted from the play. We can look at each teams defensive EPA and offensive EPA for the 2021 season. 
```{r}
# read in pbp data
pbp=read.csv('~/Google Drive/My Drive/Sports Analytics/SportsAnalyticsBook/data/nfl_pbp_2021.gz')
pbp <- select(pbp, c('posteam','epa','yards_gained','defteam'))

# get offensive epa and epa per play
off_df <- pbp %>%
  group_by(posteam) %>%
  summarise(off_epa = sum(epa),
            off_plays = n(),
            off_epp = off_epa/off_plays,
            .groups='drop')

# get defensive epa and epa per play
def_df <- pbp %>%
  group_by(defteam) %>%
  summarise(def_epa = sum(epa),
            def_plays = n(),
            def_epp = def_epa/def_plays,
            .groups='drop')
# merge dataframes
names(off_df)[names(off_df) == 'posteam'] <- 'team'
names(def_df)[names(def_df) == 'defteam'] <- 'team'
epa_df <- merge(off_df,def_df,by='team')
epa_df <- na.omit(epa_df)

# plot
ggplot(epa_df,aes(x=off_epp,y=def_epp, label=team))+geom_point()+geom_hline(yintercept=0)+geom_vline(xintercept=0)+geom_label(aes(label = team))+xlab('Offensive EPA per Play')+ylab('Defensive EPA per Play')
```
...
## UNFINISHED IDEAS (for both classes)

### Football

#### Average Expected Points by Field Position
...
#### Difference of means Nadal on clay vs other surfaces

#### Linear Regression contrast nadal on clay vs other surfaces


### Basketball

#### Difference in means clutch basketball moments (high leverage situation vs low leverage situation)


#Baseball

#### Random Forest pitch classification

#### Scrape daily lineups from baseballpress.com and get summary statistics from baseballr or other package

#### How relevant is 1st inning pitching performance to the rest of the games performance?

#### Comparing difference of means of lefty pitchers vs righties


### Hockey

#### Difference in means scoring between 2nd and 3rd period


### Misc

#### Assignemnt idea where the main purpose is creating interesting visualizations from a dataset since conveying statistical information to others is often as important as understanding it yourself
