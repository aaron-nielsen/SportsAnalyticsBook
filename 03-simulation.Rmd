...
FT %>% as.data.frame() %>% ggplot(aes(x=FT)) + geom_bar()
...
In this case, we could have also calculated the exact probability of interest (0.00165) using binomial random variable.
...
games.sim <- round( rnorm(n=n.sims,mean = games.mean, sd = games.sd))

# number of games can't be negative
games.sim[games.sim<0] = 0
...
for(i in 1:n.sims){
  ...
  temp <- round( rnorm(n = games.sim[i],mean = ft.mean, sd = ft.sd))
  FT[i] <- sum(temp)
}
